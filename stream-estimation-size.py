...
def analyze_lines_multiple_sketches(lines, distributions, servers):
    ...
    with open(os.path.join("Pickle","pickle_{}_{}.bin".format(servers, len(src_ip_dict))), "wb") as f:
        pickle.dump(split_sketches, f)
    k = 512
    base_sketch.resize(k)
    base_sketch_estimation = base_sketch.get_size()
    sketches_estimations = []
    for sketches in split_sketches:
        for s in sketches:
        ...
        servers_needed_ratios = [math.ceil((s*k)/total_servers_estimation) for s in servers_estimations]
...
def main():
    ...
    j = 0
    servers = 100
    distributions = [generate_linear_distribution(5,servers), generate_exponential_distribution(5,servers), generate_arc_tan_distribution(5,servers), generate_constant_distribution(servers)]
    results = {}
    ...
    for file_i, filename in enumerate(ip_files):
        ...
        with open(full_filename, "r") as f:
            ...
            for line in f.readlines():
                ...
                lines.append(line)
                if i %1000000 == 0:
                    analyze_lines_multiple_sketches(lines, distributions, servers)
                    i=0
                    lines = []
                    j+=1
                    key_hashes = {}
